import { buildDeck, compareCutCards, shuffleDeck, upcardKnockValue, } from "./cards.js";
import { applyLayoffsGreedy, bestAfterDiscard11, bestDeadwood, isBigGin11, isValidMeld, } from "./melds.js";
import { computeBettingSettlement, deadwoodTotal, resolveKnockFinal, scoreEO, scoreGin, validateKnockerLayout, } from "./scoring.js";
function cloneState(s) {
    return JSON.parse(JSON.stringify(s));
}
function markSeen(state, card, viewers) {
    const cur = state.seenBy[card] ?? [false, false];
    const next = [cur[0] ?? false, cur[1] ?? false];
...
export function createNewMatch(_seed, rng) {
    const deck = shuffleDeck(buildDeck(), rng);
    const firstSeat = (rng() < 0.5 ? 0 : 1);
    return {
        version: 1,
        phase: "cutForDeal",
        handIndex: 0,
...
function dealHandFromPile(state, pile) {
    state.hands = [[], []];
    for (let i = 0; i < 10; i++) {
...
function awardHand(state, winner, points) {
    state.scores[winner] += points;
    state.handsWon[winner] += 1;
    state.lastHandWinner = winner;
    state.lastHandPoints = points;
    state.phase = "handOver";
    const w = state.scores[0] >= state.raceTarget ? 0 : state.scores[1] >= state.raceTarget ? 1 : null;
    if (w !== null) {
...
function maybeStartNextHand(state, rng) {
    if (state.phase !== "handOver")
        return;
    const winner = state.lastHandWinner;
    if (winner === null)
        return;
    if (state.scores[0] >= state.raceTarget || state.scores[1] >= state.raceTarget)
        return;
    state.handIndex += 1;
    state.dealer = winner;
...
export function applyIntent(state, intent, rng) {
...
function applyCutPick(state, seat, index) {
    ...
    const firstSeat = (state.cut.firstSeat ?? 0);
    const secondSeat = otherSeat(firstSeat);
    const expected = state.cut.picks[firstSeat] === null
        ? firstSeat
...
function applyUpcardTake(state, seat) {
    ...
    if (stage === "nonDealer") {
        if (seat !== nd)
            return { ok: false, error: "Non-dealer acts first" };
    ...
    if (stage === "dealer") {
        if (seat !== d)
            return { ok: false, error: "Dealer acts" };
        if (!nonDealerPassed)
            return { ok: false, error: "Non-dealer has not passed" };
        const up = state.discard[state.discard.length - 1];
        state.hands[seat].push(up);
        state.discard.pop();
        markSeen(state, up, [0, 1]);
...
function applyUpcardPass(state, seat) {
    if (state.phase !== "upcardOffer" || !state.upcardOffer) {
        return { ok: false, error: "Not in upcard offer" };
    }
    const nd = state.nonDealer;
    ...
    if (state.upcardOffer.stage === "nonDealer") {
        if (seat !== nd)
            return { ok: false, error: "Wrong seat" };
    ...
    if (state.upcardOffer.stage === "dealer") {
        if (seat !== d)
            return { ok: false, error: "Wrong seat" };
        if (!state.upcardOffer.nonDealerPassed)
            return { ok: false, error: "Invalid pass order" };
        state.upcardOffer = null;
        state.phase = "play";
...
function applyDrawStock(state, seat) {
    if (state.phase === "play") {
        if (state.currentTurn !== seat)
            return { ok: false, error: "Not your turn" };
        if (state.hands[seat].length !== 10)
            return { ok: false, error: "Expected 10 cards before draw" };
    ...
    if (state.phase === "upcardOffer") {
        const nd = state.nonDealer;
        if (seat !== nd)
            return { ok: false, error: "Only non-dealer draws after two passes" };
        ...
        if (state.hands[seat].length !== 10)
            return { ok: false, error: "Invalid hand size" };
        if (state.stock.length === 0)
            return { ok: false, error: "Stock empty" };
        const c = state.stock.pop();
        state.hands[seat].push(c);
...
function applyTakeDiscard(state, seat) {
    if (state.phase !== "play") {
...
function applyDiscard(state, intent) {
    ...
    if (hand.length !== 11)
        return { ok: false, error: "Must have 11 cards to discard" };
    if (!hand.includes(card))
        return { ok: false, error: "Card not in hand" };
    const hand10 = hand.filter((c) => c !== card);
    if (hand10.length !== 10)
        return { ok: false, error: "Hand size error" };
     ...
     */
    let resolvedKnockMelds = null;
    let resolvedKnockDeadwood = null;
    if (gin) {
        const { sum } = bestDeadwood(hand10);
        if (sum !== 0)
            return { ok: false, error: "Gin not legal" };
    }
    else if (knock) {
        /**
         ...
         */
        if (knockVal === null)
            return { ok: false, error: "Cannot knock when knock card is an Ace" };
        if (layout) {
    ...
    if (knock && resolvedKnockMelds && resolvedKnockDeadwood) {
        ...
        for (const c of state.knock.knockerDeadwood)
            markSeen(state, c, [0, 1]);
        return { ok: true, state };
    }
...
function applyDeclareBigGin(state, seat) {
    if (state.phase !== "play")
        return { ok: false, error: "Not in play" };
    if (state.currentTurn !== seat)
        return { ok: false, error: "Not your turn" };
    const hand = state.hands[seat];
    if (hand.length !== 11)
        return { ok: false, error: "Need 11 cards for EO" };
    if (!isBigGin11(hand))
        return { ok: false, error: "Not EO" };
    const opp = otherSeat(seat);
...
function applyLayoffAttach(state, intent) {
    if (intent.type !== "layoffAttach")
        return { ok: false, error: "Internal" };
    if (state.phase !== "knockLayoff" || !state.knock)
        return { ok: false, error: "No knock layoff" };
    const { seat, card, meldIndex } = intent;
    if (state.knock.layoffTurn !== seat)
        return { ok: false, error: "Not layoff turn" };
    const knocker = state.knock.knocker;
    if (seat === knocker)
        return { ok: false, error: "Knocker cannot lay off" };
    const idx = state.knock.opponentDeadwood.indexOf(card);
    if (idx < 0)
        return { ok: false, error: "Card not in opponent deadwood" };
    const melds = state.knock.knockerMeldsAfterLayoff;
    const meld = melds[meldIndex];
    if (!meld)
        return { ok: false, error: "Invalid meld index" };
    const trial = { ...meld, cards: [...meld.cards, card] };
    if (!isValidMeld(trial))
        return { ok: false, error: "Illegal layoff" };
    meld.cards.push(card);
    state.knock.opponentDeadwood.splice(idx, 1);
    const hi = state.hands[seat].indexOf(card);
    if (hi >= 0)
        state.hands[seat].splice(hi, 1);
    markSeen(state, card, [0, 1]);
    return { ok: true, state };
}
function applyLayoffDone(state, seat) {
    if (state.phase !== "knockLayoff" || !state.knock)
        return { ok: false, error: "No knock layoff" };
    if (state.knock.layoffTurn !== seat)
        return { ok: false, error: "Not layoff turn" };
    const knocker = state.knock.knocker;
    if (seat === knocker)
        return { ok: false, error: "Knocker cannot finish layoff" };
    /**
    ...
    const { melds: newMelds, opponentDeadwood: newDeadwood } = applyLayoffsGreedy(state.knock.knockerMeldsAfterLayoff, beforeDeadwood);
